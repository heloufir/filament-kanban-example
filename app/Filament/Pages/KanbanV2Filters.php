...
class KanbanV2Filters extends KanbanBoard
{
    protected static ?string $navigationIcon = 'heroicon-o-view-columns';

    protected static ?string $slug = 'v2/filters';

    protected static ?string $navigationGroup = 'Version 2';

    protected static ?string $title = 'Kanban Board Filters';

    protected bool $shouldShowFilters = true;
    ...
    {
        if (isset($this->filtersData['title'])) {
            $query->where('title', 'like', '%' . $this->filtersData['title'] . '%');
        }
        if (isset($this->filtersData['progress'])) {
            $query->where('progress', '>=', $this->filtersData['progress']);
        }
        if (isset($this->filtersData['deadline'])) {
            $query->whereDate('deadline', '>=', $this->filtersData['deadline']);
        }
        ...
        return [
            Grid::make()
                ...
                ->schema([
                    ...
                    TextInput::make('progress')
                        ->numeric()
                        ->placeholder('Having progress more than'),
